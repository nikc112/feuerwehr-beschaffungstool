def test_proposal_without_hersteller_modell_accepted(app, client, monkeypatch):
    ...
    # Bezeichnung + E-Mail gesetzt – Hersteller/Modell sind kein Pflichtfeld
...
def test_proposal_without_bezeichnung_rejected(app, client, monkeypatch):
    monkeypatch.setattr('app.api.notify_new_proposal', lambda *a, **k: None)
    r = client.post('/api/proposals',
                    data={'hersteller': 'FLIR', 'modell': 'K33', 'einreicher_email': 'melder@example.com'},
                    content_type='multipart/form-data')
    assert r.status_code == 400


def test_proposal_requires_einreicher_email(app, client, monkeypatch):
    monkeypatch.setattr('app.api.notify_new_proposal', lambda *a, **k: None)
    # ohne E-Mail -> abgelehnt
    assert client.post('/api/proposals', data={'bezeichnung': 'Kamera'},
                       content_type='multipart/form-data').status_code == 400
    # ungültige E-Mail -> abgelehnt
    assert client.post('/api/proposals', data={'bezeichnung': 'Kamera', 'einreicher_email': 'keine-mail'},
                       content_type='multipart/form-data').status_code == 400
    # mit gültiger E-Mail -> angenommen
    assert client.post('/api/proposals', data={'bezeichnung': 'Kamera', 'einreicher_email': 'a@b.de'},
...
def test_beschaffer_can_add_hersteller_modell_later(app, auth_client, monkeypatch):
    ...
    nr = auth_client.post('/api/proposals',
                          data={'bezeichnung': 'Wärmebildkamera', 'einreicher_email': 'melder@example.com'},
